...
import cn.edu.whu.spatialJoin.spatialPartitioning.quadtree.StandardQuadTree;
import cn.edu.whu.spatialJoin.spatialRddTool.IndexBuilder;
import cn.edu.whu.spatialJoin.spatialRddTool.StatCalculator;
...
import cn.edu.whu.spatialJoin.utils.RDDSampleUtils;
...
public class SpatialRDD<T extends Geometry>
    ...
    public boolean spatialPartitioning(PartitionerType partitionerType)
            throws Exception {
        int numPartitions = this.rawSpatialRDD.rdd().partitions().length;
        spatialPartitioning(partitionerType, numPartitions);
        return true;
    }

    public boolean spatialPartitioning(PartitionerType partitionerType, Envelope boundaryEnvelope, List<Envelope> samples)
            throws Exception {
        int numPartitions = this.rawSpatialRDD.rdd().partitions().length;
        spatialPartitioning(partitionerType, numPartitions,boundaryEnvelope,samples);
        return true;
    }
...
//                return AdaptiveGrid.transformGridGeometry(geometry);
     ...
     *
     * @param partitionerType the grid type
     * @return true, if successful
     * @throws Exception the exception
     */
    public void spatialPartitioning(PartitionerType partitionerType, int numPartitions)
            throws Exception {
        if (numPartitions <= 0) {
        ...
        //System.out.println("spatialPartitioning : " + gridType + "  " + numPartitions + "  " + sampleNumberOfRecords + "  " + getClass());
        ...
        switch (partitionerType) {
            ...
            case KDBTREE: {
                KDBTreePartitioning tree = new KDBTreePartitioning(samples, paddedBoundary, numPartitions);
                partitioner = new KDBTreePartitioner(tree.getPartitionTree());
                break;
            }
    ...
    public void spatialPartitioning(PartitionerType partitionerType, int numPartitions,Envelope boundaryEnvelope, List<Envelope> samples)
            throws Exception {

        switch (partitionerType) {
            case QUADTREE: {
                QuadtreePartitioning quadtreePartitioning = new QuadtreePartitioning((List<Envelope>) samples, boundaryEnvelope, numPartitions);
                partitioner = new QuadTreePartitioner(quadtreePartitioning.getPartitionTree());
                break;
            }

            case KDBTREE: {
                KDBTreePartitioning tree = new KDBTreePartitioning(samples, boundaryEnvelope, numPartitions);
                partitioner = new KDBTreePartitioner(tree.getPartitionTree());
                break;
            }
        ...
        return this.rawSpatialRDD.flatMapToPair(
                new PairFlatMapFunction<T, Integer, T>() {
                    ...
                    public Iterator<Tuple2<Integer, T>> call(T spatialObject)
                            throws Exception {
                        //System.out.println(Arrays.toString(((GridLineString)spatialObject).getGrids()));
    ...
    public boolean analyze() {
        ...
        // System.out.println("数量 ：" + rawSpatialRDD.count());
        // 每个分区先进行seqOp操作，然后再用combOp对所有分区操作
    ...
    /**
     * Gets the CRS transformation.
     *
     * @return the CRS transformation
     */
