...
public class PIPJudgement<T extends Geometry>
     ...
     * @see JudgementBase
     */
    public PIPJudgement(boolean considerBoundaryIntersection, @Nullable DedupParams dedupParams) {
    ...
    public Iterator<PartResultStatistic> call(Iterator<SpatialIndex> indexIterator,Iterator<T> streamShapes)
        ...
        while (streamShapes.hasNext()) {
            ...
            for (Geometry candidate : candidates) {
                if(!(candidate.getEnvelopeInternal().intersects(streamShape.getEnvelopeInternal()))){ continue; }
                refineCost += (candidate.getNumPoints()+streamShape.getNumPoints()-1);
                toRefineNum += 1;
