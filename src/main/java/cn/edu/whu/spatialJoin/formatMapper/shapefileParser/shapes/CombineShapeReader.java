...
package cn.edu.whu.spatialJoin.formatMapper.shapefileParser.shapes;

import cn.edu.whu.spatialJoin.formatMapper.shapefileParser.parseUtils.shp.ShapeType;
    ...
    public void initialize(InputSplit split, TaskAttemptContext context)
        ...
        for (int i = 0; i < paths.length; ++i) {
            ...
            if (suffix.equals(SHP_SUFFIX)) {
                shpSplit = new FileSplit(paths[i], fileSplit.getOffset(i), fileSplit.getLength(i), fileSplit.getLocations());
            } else if (suffix.equals(SHX_SUFFIX)) {
                shxSplit = new FileSplit(paths[i], fileSplit.getOffset(i), fileSplit.getLength(i), fileSplit.getLocations());
            } else if (suffix.equals(DBF_SUFFIX)) {
                dbfSplit = new FileSplit(paths[i], fileSplit.getOffset(i), fileSplit.getLength(i), fileSplit.getLocations());
            }
        }
